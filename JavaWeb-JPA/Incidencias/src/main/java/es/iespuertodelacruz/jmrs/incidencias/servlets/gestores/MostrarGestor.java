...
/**
 * Servlet implementation class MostrarGestor
 */
public class MostrarGestor extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MostrarGestor() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String idgestorMostrar= request.getParameter("gestorId");
		if(idgestorMostrar != null && !idgestorMostrar.trim().isEmpty()) {
			gestor = gestorDao.findById(idgestorMostrar);
			
			if(gestor != null) {
				String idGestor = gestor.getDNI();
				String nombre = gestor.getNombre();
				String apellidos = gestor.getApellidos();
				request.getSession().setAttribute(Globals.MENSAJE_MOSTRAR, idGestor + ", " 
						+ nombre + ", "
						+ apellidos);
				try {
				...
				request.getSession().setAttribute(Globals.MENSAJE_MOSTRAR, "El gestor con id: " + idgestorMostrar + " NO se ha podido encontrar");
				request.getRequestDispatcher("gestores.jsp").forward(request, response);
			}
		} else {
